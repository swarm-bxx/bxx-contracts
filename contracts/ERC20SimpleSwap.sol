...
contract ERC20SimpleSwap {
    ...
    event HardDepositTimeoutChanged(
        address indexed beneficiary,
        uint256 timeout
    );
    event Withdraw(uint256 amount);

    uint256 public defaultHardDepositTimeout;
    /* structure to keep track of the hard deposits (on-chain guarantee of solvency) per beneficiary*/
    struct HardDeposit {
        uint256 amount; /* hard deposit amount allocated */
        uint256 decreaseAmount; /* decreaseAmount substranced from amount when decrease is requested */
        uint256 timeout; /* issuer has to wait timeout seconds to decrease hardDeposit, 0 implies applying defaultHardDepositTimeout */
        uint256 canBeDecreasedAt; /* point in time after which harddeposit can be decreased*/
    }
    ...
    function domainSeparator(EIP712Domain memory eip712Domain)
        internal
        pure
        returns (bytes32)
    {
    ...
    function recoverEIP712(bytes32 hash, bytes memory sig)
        internal
        view
        returns (address)
    {
    ...
    ERC20 public token;
    I_MiningReward public miningReward;
  ...
  */
    function init(
        address _issuer,
        address _token,
        address _miningReward,
        uint256 _defaultHardDepositTimeout
    ) public {
        require(_issuer != address(0), "invalid issuer");
        require(issuer == address(0), "already initialized");
        issuer = _issuer;
        token = ERC20(_token);
        miningReward = I_MiningReward(_miningReward);
        defaultHardDepositTimeout = _defaultHardDepositTimeout;
        if (_token != address(0) && _miningReward != address(0)) {
            token.approve(_miningReward, miningReward.STAKING_AMOUNT());
        }
    }
    ...
    function liquidBalanceFor(address beneficiary)
        public
        view
        returns (uint256)
    {
  ...
  */
    function _cashChequeInternal(
        ...
        miningReward.join(beneficiary, issuer);
  ...
  */
    function prepareDecreaseHardDeposit(
        ...
        uint256 timeout = hardDeposit.timeout == 0
  ...
  */
    function increaseHardDeposit(address beneficiary, uint256 amount) public {
        require(msg.sender == issuer, "SimpleSwap: not issuer");
